...
export const ShortUrlCreationSettings: FC<ShortUrlCreationProps> = ({ onChange }) => {
  ...
  const changeTagsFilteringMode = (tagFilteringMode: TagFilteringMode) => () =>
    onChange({ ...shortUrlCreation, tagFilteringMode });
  ...
  return (
    <SimpleCard title="Short URLs form" className="card" bodyClassName="flex flex-col gap-4">
      <LabelledToggle
        ...
        helpText={
          <>
            The initial state of the <b>Forward query params on redirect</b> checkbox will be{' '}
            <b>{(shortUrlCreation?.forwardQuery ?? true) ? 'checked' : 'unchecked'}</b>.
          </>
        }
      >
      ...
      <div className="flex flex-col">
        ...
        <Dropdown buttonContent={tagFilteringModeText(shortUrlCreation?.tagFilteringMode)} buttonClassName="w-full">
          <Dropdown.Item
        ...
        <Muted size="sm" className="mt-0.5">
