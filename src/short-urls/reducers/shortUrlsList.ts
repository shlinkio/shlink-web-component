...
export const listShortUrlsThunk = createAsyncThunk(
  ...
  ({ apiClientFactory, ...params }: WithApiClient<ShlinkShortUrlsListParams>): Promise<ShlinkShortUrlsList> =>
    apiClientFactory().listShortUrls(params ?? {}),
);
...
export const { reducer: shortUrlsListReducer } = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(listShortUrlsThunk.fulfilled, (_, { payload: shortUrls }) => ({ status: 'loaded', shortUrls }));
    ...
    builder.addCase(editShortUrlThunk.fulfilled, (state, { payload: editedShortUrl }) => {
      if (state.status !== 'loaded') {
        return;
      }
    ...
    builder.addCase(shortUrlDeleted, (state, { payload }) => {
      if (state.status !== 'loaded') {
        return;
      }
    ...
    builder.addCase(createNewVisits, (state, { payload }) => {
      ...
      state.shortUrls.data = state.shortUrls.data.map(
        ...
        // amount of visits.
        (currentShortUrl) =>
          payload.createdVisits.findLast(
